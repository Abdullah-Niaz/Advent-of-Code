...
class Solution:
        ...
        while gate_connections:
            ...
            for i, line in enumerate(gate_connections):
                ...
                if wires[input1] is not None and wires[input2] is not None:
                    if gate == "AND":
                        wires[output] = 1 if wires[input1] + \
                            wires[input2] == 2 else 0
                    elif gate == "OR":
                        wires[output] = 1 if wires[input1] + \
                            wires[input2] > 0 else 0
                    elif gate == "XOR":
                        wires[output] = 1 if wires[input1] != wires[input2] else 0
                    done.append(i)
            gate_connections = [v for i, v in enumerate(
                gate_connections) if i not in done]

        z_wires = [v for k, v in sorted([val for val in wires.items(
        ) if val[0][0] == "z"], key=lambda x: x[0], reverse=True)]
    ...
    def part2(self, data):
        ...
        x_bin = "".join([line.split(": ")[1]
                        for line in init_wire_values if line.startswith("x")])[::-1]
        y_bin = "".join([line.split(": ")[1]
                        for line in init_wire_values if line.startswith("y")])[::-1]
        ...
        while not is_valid:
            ...
            z_test = bin(self.part1(
                data[: pos + 1] + curr_gate_connections))[2:][::-1]
            ...
            for i in range(len(z_bin)):
                if z_bin[i] != z_test[i] or len(swap_candidate) == 1:
                    ...
                    connections = self.print_gate_connections(
                        gate_relation, key1)
                    print(connections)
                    ...
                    print("invalid format, need swap:",
                          swap_candidate[-1], end="\n\n")

                    if len(swap_candidate) == 2:
                        break

            gate_relation[swap_candidate[0]], gate_relation[swap_candidate[1]
                                                            ] = gate_relation[swap_candidate[1]], gate_relation[swap_candidate[0]]
