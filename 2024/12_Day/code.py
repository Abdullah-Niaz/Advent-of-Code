    ...
    def part1(self, data):
        ...
        for _type in types:
            ...
            while coords:
                ...
                price = len(curr_group) * (len(curr_group) *
                                           4 - sum(connected_count.values()))
                prices[_type] += price
        ...
        def get_group_sides(group):
            ...
            for _ in range(2):
                for y in range(1, rows + 1):
                    sides += len("".join(["X" if current != above and current ==
                                 "X" else " " for current, above in zip(grid[y], grid[y - 1])]).split())
                    sides += len("".join(["X" if current != above and current ==
                                 "X" else " " for current, above in zip(grid[y], grid[y + 1])]).split())
