...
class Solution:
            ...
            for key2 in keys:
                ...
                if p1[0] == p2[0]:
                    ...
                    if p1[0] != pos["#"][0] or p2[1] != pos["#"][1]:
                        ...
                        moves[(key1, key2)].append(
                            d1 * (abs(p2[1] - p1[1])) + d2 * (abs(p2[0] - p1[0])) + "A")
                    if p1[1] != pos["#"][1] or p2[0] != pos["#"][0]:
                        d1 = "v" if p2[0] > p1[0] else "^"
                        d2 = ">" if p2[1] > p1[1] else "<"
                        moves[(key1, key2)].append(
                            d1 * (abs(p2[0] - p1[0])) + d2 * (abs(p2[1] - p1[1])) + "A")
        return moves
    ...
    def build_combinations(self, arrays, current=[], index=0):
        ...
        for value in arrays[index]:
            new_results = self.build_combinations(
                arrays, current + [value], index + 1)
    ...
    def translate_keypad(self, code):
        code = "A" + code
        moves = [self.moves2[(a, b)] if a != b else ["A"]
